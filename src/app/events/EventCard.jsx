...
const EventCard = ({ item }) => {
  ...
  return (
    <Link href={`/events/${item.id}`}>
      ...
      >
        <div className="absolute left-0 bottom-0 w-full h-2/5 bg-black bg-opacity-70 backdrop-blur-sm flex items-start text-white px-2 flex-col justify-between py-4">
          <h3>{shortTitle}</h3>
          <p className="text-xs">
            {item.date} {item.time}
          </p>
